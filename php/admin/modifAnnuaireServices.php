...
if ($numero != "000000000") {
    $fieldsToUpdate[] = "numPort = ?";
    $params[] = $numero;
    $types .= "s";
}
if ($numeroext != "000000000") {
    $fieldsToUpdate[] = "numExt = ?";
    $params[] = $numeroext;
    $types .= "s";
}
if ($dirfixe != "0") {
    $fieldsToUpdate[] = "dirFixe = ?";
    $params[] = $dirfixe;
    $types .= "s";
}
...
$params[] = $nomAnnuaire;
$params[] = $prenomAnnuaire;
$types .= "ss";

if (!empty($fieldsToUpdate)) {
    $sql = "UPDATE services SET " . implode(", ", $fieldsToUpdate) . " WHERE nom = ? AND prenom = ?";
    $stmt = $mysqli->prepare($sql);
